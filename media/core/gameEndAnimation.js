...
function createBadge({ square, className, iconKey, label, squareSizePx }) {
  const el = document.createElement("div");
  el.className = `animated-effect ${className}`;
  ...
  el.innerHTML = `
    ...
    <div class="icon-background ${className}"></div>
...
export function showGameEndBadges() {
  ...
  if (game.isCheckmate()) {
    ...
    const loserClass =  loserColor  === "w" ? "checkmatewhite white" : "checkmateblack black";
    const winnerClass = winnerColor === "w" ? "white" : "black";

    if (winnerSquare) {
      gameEndLayer.append(createBadge({
        square: winnerSquare, 
        className: `winner ${winnerClass}`,
        iconKey: "winner", 
        label: "Winner", 
        squareSizePx,
      }));
